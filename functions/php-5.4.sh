...
sed -i 's@^/usr/local/php/@/usr/local/php54/@' $php_install_dir/etc/php.ini
[ -e /usr/sbin/sendmail ] && sed -i 's@^;sendmail_path.*@sendmail_path = /usr/sbin/sendmail -t -i@' $php_install_dir/etc/php.ini

if [ "$Apache_version" != '1' -a "$Apache_version" != '2' ];then
# php-fpm54 Init Script
/bin/cp sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm54
chmod +x /etc/init.d/php-fpm54
OS_CentOS='chkconfig --add php-fpm54 \n
chkconfig php-fpm54 on'
OS_Debian_Ubuntu='update-rc.d php-fpm54 defaults'
OS_command
