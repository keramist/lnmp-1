...
if [ "$Apache_version" == '1' -o "$Apache_version" == '2' ];then
        ...
        /bin/cp conf/php53.conf $tengine_install_dir/conf/php53.conf
fi

sed -i "s@/home/web/default@$home_dir/default@" $tengine_install_dir/conf/nginx.conf
sed -i "s@/home/weblogs@$wwwlogs_dir@g" $tengine_install_dir/conf/nginx.conf
...
sed -i "s@/home/web@$home_dir@g" vhost.sh
sed -i "s@/home/weblogs@$wwwlogs_dir@g" vhost.sh
ldconfig
service nginx start
}
