...
# php-fpm7 Init Script
/bin/cp sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm7
chmod +x /etc/init.d/php-fpm7
OS_CentOS='chkconfig --add php-fpm7 \n
chkconfig php-fpm7 on'
OS_Debian_Ubuntu='update-rc.d php-fpm7 defaults'
OS_command
...
[ "$Web_yn" == 'n' ] && sed -i "s@^listen =.*@listen = $local_IP:9007@" $php_install_dir/etc/php-fpm.conf 
