...
src_url=https://downloads.mariadb.org/interstitial/mariadb-galera-$mariadb_version/source/mariadb-galera-$mariadb_version.tar.gz && Download_src 

useradd -M -s /sbin/nologin mysql
mkdir -p $mariadb_data_dir $mariadb_install_dir /var/run/mysqld
chown mysql.mysql -R $mariadb_data_dir
chown mysql.mysql -R $mariadb_install_dir
chown mysql.mysql -R /var/run/mysqld

tar zxf mariadb-galera-$mariadb_version.tar.gz
...
if [ -d "$mariadb_install_dir" ];then
        echo -e "\033[32mMariaDB Galera install successfully! \033[0m"
else
        echo -e "\033[31mMariaDB Galeta install failed, Please contact the author! \033[0m"
        kill -9 $$
fi
...
OS_command
cd ..
/bin/rm -rf mariadb-galera-$mariadb_version
cd ..
