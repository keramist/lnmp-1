...
if [ "$DB_ISR" != 'repo' ];then
  ln -s $db_install_dir/lib/libmysqlclient.so /usr/lib
...
./configure --prefix=$pureftpd_install_dir CFLAGS=-O2 --with-mysql --with-quotas --with-cookie --with-virtualhosts --with-virtualchroot --with-diraliases --with-sysquotas --with-ratios --with-altlog --with-paranoidmsg --with-shadow --with-welcomemsg  --with-throttling --with-uploadscript --with-language=english --with-rfc2640
fi
...
chkconfig pureftpd on'
	OS_Debian_Ubuntu="sed -i 's@^. /etc/init.d/functions@. /lib/lsb/init-functions@' /etc/init.d/pureftpd \n
