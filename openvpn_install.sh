...
if [ -e /etc/openvpn/server.conf ]; then
	...
	clear
		...
		case $option in
			...
			cp /etc/openvpn/easy-rsa2/keys/ca.crt ~/ovpn-$CLIENT
			cp /etc/openvpn/easy-rsa2/keys/$CLIENT.crt ~/ovpn-$CLIENT
			cp /etc/openvpn/easy-rsa2/keys/$CLIENT.key ~/ovpn-$CLIENT
			cd ~/ovpn-$CLIENT
			...
			. /etc/openvpn/easy-rsa2/revoke-full $CLIENT
			...
			if grep -q "crl-verify" "/etc/openvpn/server.conf"; then
				...
				echo "crl-verify /etc/openvpn/easy-rsa2/keys/crl.pem" >> "/etc/openvpn/server.conf"
			...
			sed -i '/iptables -t nat -A POSTROUTING -s 10.8.8.0/d' /etc/rc.local
			echo ""
	...
	if [ ! -d /etc/openvpn/easy-rsa2/ ]; then
		#wget --no-check-certificate -O ~/easy-rsa.tar.gz https://github.com/OpenVPN/easy-rsa/archive/2.2.2.tar.gz
		#tar xzf ~/easy-rsa.tar.gz -C ~/
		wget --no-check-certificate -O ~/easy-rsa-release-2.x.zip https://github.com/OpenVPN/easy-rsa/archive/release/2.x.zip
		unzip ~/easy-rsa-release-2.x.zip
		mkdir -p /etc/openvpn/easy-rsa2/
		cp ~/easy-rsa-release-2.x/easy-rsa/2.0/* /etc/openvpn/easy-rsa2/
		rm -rf ~/easy-rsa-release-2.x
	fi
	cd /etc/openvpn/easy-rsa2/
	cp -u -p openssl-1.0.0.cnf openssl.cnf
	# update key size to 2048 bits
	sed -i 's|export KEY_SIZE=1024|export KEY_SIZE=2048|' /etc/openvpn/easy-rsa2/vars
	# Create the PKI
	. /etc/openvpn/easy-rsa2/vars
	...
	# Set iptables
	iptables -t nat -A POSTROUTING -s 10.8.8.0/24 -j SNAT --to $IP
	sed -i "/# By default this script does nothing./a\iptables -t nat -A POSTROUTING -s 10.8.8.0/24 -j SNAT --to $IP" /etc/rc.local
