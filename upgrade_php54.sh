...
cd src
. ../options.conf
[ ! -e "/usr/local/php54" ] && echo -e "\033[31mThe PHP is not installed on your system!\033[0m " && exit 1
...
echo
Old_php_version=`/usr/local/php54/bin/php -r 'echo PHP_VERSION;'`
...
if [ -e "php-$php_version.tar.gz" ];then
	...
	patch -d php-$php_version -p0 < fpm-race-condition.patch
        cd php-$php_version
	make clean
        /usr/local/php54/bin/php -i |grep 'Configure Command' | awk -F'=>' '{print $2}' | bash
