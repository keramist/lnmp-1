...
#       http://lnmp.yunnan.ws
#       https://github.com/tekintian/ltmp
        ...
        if [ "$Web_yn" != 'y' -a "$Web_yn" != 'n' ];then
                ...
                if [ "$Web_yn" == 'y' ];then
                        ...
                        do
                                ...
                                read -p "Please input a number:(Default 2 press Enter) " Nginx_version
                                [ -z "$Nginx_version" ] && Nginx_version=2
                ...
                if [ "$DB_yn" == 'y' ];then
                    #    [ -d "$db_install_dir" ] && { echo -e "\033[31mThe database already installed! \033[0m" ; DB_yn=n ; break ; }
                        ...
                        do
                                ...
                                echo -e "\t\033[32m1\033[0m. Install MariaDB-10.0"
                                echo -e "\t\033[32m2\033[0m. Install MariaDB-Galera-10.0"
                                echo -e "\t\033[32m3\033[0m. Install MariaDB-5.5"
                                echo -e "\t\033[32m4\033[0m. Install MariaDB-Galera-5.5"
                                echo -e "\t\033[32m5\033[0m. Install MySQL-5.6"
                                echo -e "\t\033[32m6\033[0m. Install MySQL-5.5"
                                echo -e "\t\033[32m7\033[0m. Install Percona-5.6"
                                echo -e "\t\033[32m8\033[0m. Install Percona-5.5"
...
# Database
if [ "$DB_ISR" == 'repo' ];then
sed -i 's@^DB_ISR.*@DB_ISR=repo@' $lnmp_dir/options.conf
if [ "$DB_version" == '1' ];then
    . functions/mariadb-10.0-repo.sh
	Install_MariaDB-10-0-repo 2>&1 | tee -a $lnmp_dir/install.log
elif [ "$DB_version" == '2' ];then
      . functions/mariadb-galera-10.0-repo.sh
	Install_MariaDB-Galera-10-0-repo 2>&1 | tee -a $lnmp_dir/install.log 
elif [ "$DB_version" == '3' ];then
	. functions/mariadb-5.5-repo.sh
	Install_MariaDB-5-5-repo 2>&1 | tee -a $lnmp_dir/install.log
elif [ "$DB_version" == '4' ];then
	. functions/mariadb-galera-5.5-repo.sh
	Install_MariaDB-Galera-5-5-repo 2>&1 | tee -a $lnmp_dir/install.log 
elif [ "$DB_version" == '5' ];then
	. functions/mysql-5.6-repo.sh
	Install_MySQL-5-6-repo 2>&1 | tee -a $lnmp_dir/install.log 
elif [ "$DB_version" == '6' ];then
	. functions/mysql-5.5-repo.sh
        Install_MySQL-5-5-repo 2>&1 | tee -a $lnmp_dir/install.log
elif [ "$DB_version" == '7' ];then
        . functions/percona-5.6-repo.sh
        Install_Percona-5-6-repo 2>&1 | tee -a $lnmp_dir/install.log
elif [ "$DB_version" == '8' ];then
	. functions/percona-5.5-repo.sh 
	Install_Percona-5-5-repo 2>&1 | tee -a $lnmp_dir/install.log 
fi

else
sed -i 's@^DB_ISR.*@DB_ISR=src@' $lnmp_dir/options.conf
if [ "$DB_version" == '1' ];then
	...
	. functions/mariadb-galera-5.5.sh
	Install_MariaDB-Galera-5-5 2>&1 | tee -a $lnmp_dir/install.log 
elif [ "$DB_version" == '5' ];then
...
if [ "$FTP_yn" == 'y' ];then
   if [ "$DB_ISR" == 'repo' ];then
   . functions/pureftpd-repo.sh
	Install_PureFTPd-repo 2>&1 | tee -a $lnmp_dir/install.log 
   else
